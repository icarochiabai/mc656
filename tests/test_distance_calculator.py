...
def test_distance_default_precision():
    ...
    assert DistanceCalculator(unit="km").calculate_distance(
        ic_coordinates, ru_coordinates
    ) == round(distance_between_both_km, 1)
    assert DistanceCalculator(unit="mi").calculate_distance(
        ic_coordinates, ru_coordinates
...
def test_distance_custom_precision(precision):
    ...
    assert distance_calculator_km.calculate_distance(
        ic_coordinates, ru_coordinates
    ) == round(distance_between_both_km, precision)
    assert distance_calculator_mi.calculate_distance(
        ic_coordinates, ru_coordinates
    ) == round(distance_between_both_mi, precision)
