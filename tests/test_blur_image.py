...
def test_blur_image(mocker):
    ...
    mock_image.filter = mocker.MagicMock(side_effect=lambda f: f"blurred_with_{round(f.radius, 1)}")
    ...
    # Assertions
    mock_image_open.assert_called_once_with(mock_path)  # Ensure Image.open was called with the path
